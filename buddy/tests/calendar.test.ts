...
test('OpenAI calls existing preference tools, Python receives saved state, and only its plan is returned', async () => {
  ...
  await mocked(async () => {
    ...
    if (String(input).endsWith('/responses')) {
      assert.deepEqual(body.tools.map((t: any) => t.name), ['set_preferred_work_hours', 'set_daily_workload_limit', 'protect_time_block', 'set_break_habits', 'remove_preference', 'list_current_preferences']);
